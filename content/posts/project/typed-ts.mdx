...
Actuellement avec la librairie node elasticsearch, lorsqu'on fait une requête search, on a des types de retour très minces. Les `_souce` sont de type `unknown`. Il n'y a aucune sécurité sur les champs ou non d'index.
