...
[llms.txt](https://llmstxt.org/) est une proposition de standard pour aider les IA à comprendre le contenu d'un site web.
...
Ainsi, en allant sur la page [https://vahor.fr/project/llms-txt](https://vahor.fr/project/llms-txt), on a une belle version HTML avec du style et du JS, mais en allant sur [https://vahor.fr/project/llms-txt.md](https://vahor.fr/project/llms-txt.md), on peut voir tout ce contenu au format Markdown, beaucoup plus lisible pour une IA ou n’importe quel autre outil de scraping.

# Pourquoi un outil pour générer ces fichiers ?

Il existe déjà quelques outils pour générer tout ça, par exemple [https://llmstxt.firecrawl.dev/](https://llmstxt.firecrawl.dev/) qui permet de générer un fichier `llms.txt` ou `llms-full.txt`.
...
Ou encore [llmstxt](https://github.com/dotenvx/llmstxt).
...
Ces deux outils sont plutôt bons. Le premier utilise de l’IA pour générer les descriptions, ce qui rend la génération plus lente : quasiment 2 minutes pour mon petit test.\ 
Le deuxième est plus simple et n’a pris que 4 secondes pour générer le fichier `llms.txt`.
...
- Pouvoir modifier les titres de pages, par exemple retirer les ` | Vahor`.
...
- Pouvoir transformer le Markdown avant de l’exporter :
  - Très souvent, on utilise [remark](https://github.com/remarkjs/remark) pour enrichir le Markdown brut : par exemple, sur [vahor.fr](https://vahor.fr), les liens d’images ou autres sont relatifs au fichier Markdown (ex : `/fichier.txt`). Les garder tels quels n’est pas idéal, on veut retirer ce préfixe avant publication.
  - Retirer des champs dans la partie frontmatter. Par exemple, parfois on ajoute des tags, une configuration de sidebar, etc. Mais on souhaite peut-être ne conserver que le titre et la description.
  - Ou toute autre chose permettant de modifier le Markdown avant de l’exporter.
...
Pour [vahor.fr](https://vahor.fr), ça donne :
