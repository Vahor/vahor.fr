...
export function CodeWrapButton({ wrapperRef, ...props }: { wrapperRef: React.RefObject<HTMLDivElement | null> } & ComponentPropsWithoutRef<"button">) {
	const toggle = () => {
	...
	return (
		<Tooltip>
			<TooltipTrigger aria-label="Wrap code" {...props} onClick={toggle} className="hover:text-zinc-400 group-data-[wrap-code=true]:text-zinc-400">
				<svg role="img" aria-label="Wrap code icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={24} height={24} color="currentColor" fill="none" className="size-[18px] transform transition-transform duration-100 active:scale-95">
