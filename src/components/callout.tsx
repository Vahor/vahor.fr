...
function CalloutTemplate({
	...
	return (
		<div
			className={clsx(
				"relative my-4 overflow-hidden rounded-xl px-5 py-4",
				className,
			)}
		>
			...
			{title && (
				<h3
					className={cn(
						childrenClassName,
						"-translate-y-1 pl-7 font-semibold text-lg",
					)}
				>
					{title}
				</h3>
			)}
			<div className={clsx("prose flex-1 pl-7 text-sm", childrenClassName)}>
				{children}
			</div>
		</div>
	);
...
export function Info({ children, title }: CalloutContent) {
	return (
		<CalloutTemplate
			title={title}
			icon={({ className, ...props }) => (
				<svg
					...
					className={cn(
						"size-5 flex-none text-zinc-400 dark:text-zinc-300",
						className,
					)}
					aria-label="Info"
					{...props}
			...
			className="border border-zinc-500/20 bg-zinc-50/50 dark:border-zinc-500/30 dark:bg-zinc-500/10 dark:bg-zinc-900"
...
export function Warning({ children, title }: CalloutContent) {
	return (
		<CalloutTemplate
			title={title}
			icon={({ className, ...props }) => (
				<svg
					role="img"
					className={cn(
						"size-5 flex-none text-amber-400 dark:text-amber-300/80",
						className,
					)}
					fill="none"
					viewBox="0 0 24 24"
...
export function Note({ children, title }: CalloutContent) {
	return (
		<CalloutTemplate
			title={title}
			icon={({ className, ...props }) => (
...
export function Tip({ children, title }: CalloutContent) {
	return (
		<CalloutTemplate
			title={title}
			icon={({ className, ...props }) => (
				<svg
					...
					xmlns="http://www.w3.org/2000/svg"
					className={cn(
						"h-auto w-3.5 text-emerald-600 dark:text-emerald-400/80",
						className,
					)}
					aria-label="Tip"
					{...props}
...
export function Check({ children, title }: CalloutContent) {
	return (
		<CalloutTemplate
			title={title}
			icon={({ className, ...props }) => (
				<svg
					role="img"
					className={cn(
						"h-auto w-3.5 text-green-600 dark:text-green-400/80",
						className,
					)}
					fill="currentColor"
					xmlns="http://www.w3.org/2000/svg"
